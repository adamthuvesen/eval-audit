...
This is a **synthetic** worked example. It is not benchmark evidence. It shows
how each `decision_impact` verdict in
`eval_audit/report/decisions.py:DECISION_IMPACT_VOCAB` renders end-to-end, so a
reader can see each verdict in a real audit report instead of only in rationale
prose. The real-data audits, GAIA HAL Generalist and TAU-bench Airline Tool
Calling, carry the project's credibility. This gallery teaches the verdict
language.
...
- **`hold`**: `hold_treatment` (2/10 successes, $0.02/task) vs `hold_control` (9/10, $0.05/task). The treatment significantly underperforms control. The treatment is cheaper, so it is not Pareto-dominated; the engine reaches the *rejects-null + direction-mismatch* branch and emits `hold`. This shows the tool falsifying a claim, which matters because the engine should not behave like a confirmation machine.
