...
- The raw API responses under `scouting/exhibit-c/raw/` are the source of
...
- The harness id `eval-audit/exhibit-c-direct-v1` is unchanged. The
...
**Exhibit C: HumanEval (Anthropic Messages API, base-LLM direct-completion harness)**

Selected as the smallest credible controlled-evidence exhibit:
...
| n_tasks | 30 |
| committed task IDs | see `scouting/exhibit-c/run-plan.md` |

### Harness

| field | value |
|---|---|
| harness id | `eval-audit/exhibit-c-direct-v1` |
| version pin | git commit hash of `scouting/exhibit-c/run.py` at run time, captured in `rerun_metadata` |
| tools | none |
...
|---|---|---|---|
| treatment | `exhibit-c-sonnet-4-6` | `claude-sonnet-4-6` | temperature=0, max_tokens=1024 |
| control | `exhibit-c-haiku-4-5` | `claude-haiku-4-5-20251001` | temperature=0, max_tokens=1024 |
...
No `eval()` of model output in the parent process. Each grade runs in its own temp dir under `/tmp/exhibit-c-grade-<uuid>/`.

### Cost classification

`reconciled` when reconstructed per-task cost (from `tokens_in_by_model` × prices in `scouting/exhibit-c/price-table.yaml`) matches the Anthropic Messages API's recorded `usage` field within 0.5¢. Otherwise `as_reported_only`. The reconstructed-vs-reported reconciliation is computed per-row at normalization time.
...
Fixed 30-task list. No peeking at outcomes and extending. If decision-sensitive ambiguity remains after the 120 API calls, a follow-up exhibit (Exhibit C-2) handles it; the original is not retroactively grown.
...
- **treatment:** `exhibit-c-sonnet-4-6`
- **control:** `exhibit-c-haiku-4-5`
...
This claim is **predeclared** before any outcome is observed, in contrast with Exhibits A/B which reanalyze published runs.
...
1. Commit `scouting/exhibit-c-decision.md`, `scouting/exhibit-c/run-plan.md`, `studies/exhibit-c.yaml` (this file and its siblings) — *no API calls have been made.*
2. Vendor the 30-task HumanEval subset to `scouting/exhibit-c/humaneval-tasks-30.jsonl` from `openai/human-eval`'s release JSONL. *Still no outcomes.*
3. Commit the harness scripts (`run.py`, `grade.py`, `normalize.py`).
4. Run `scouting/exhibit-c/run.py` — *first contact with outcomes.*
5. Grade, normalize, validate, analyze, render.

If any locked field above changes after step 4, the change is a new exhibit (Exhibit C-2) with its own decision document, not an in-place edit.
