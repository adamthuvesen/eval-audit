# Exhibit C — Pre-Outcome Run Plan

**Locked:** 2026-05-03 (before any model API call has been made for this exhibit)
...
See [`../exhibit-c-decision.md`](../exhibit-c-decision.md#corrigenda-2026-05-03-pre-decision) for the full context.

This artifact is the predeclared run plan. It exists to make the controlled-evidence exhibit's design auditable: a reader can verify that the task IDs, harness, model arms, settings, cost policy, errored-row policy, and stopping rule were committed *before* any outcome was observed.

> Source-of-truth for the broader Exhibit C contract: [`scouting/exhibit-c-decision.md`](../exhibit-c-decision.md). When this run-plan and the decision document conflict, the decision document wins.

---

## Study path (planned)

`studies/exhibit-c.yaml` — committed alongside this run-plan.
...
| local vendored copy | `scouting/exhibit-c/humaneval-tasks-30.jsonl` (committed; sampled subset) |
| sampling method | `random.Random(42).sample(range(164), 30)`, sorted ascending by index |
| n_tasks | 30 |

### Locked task IDs

The 30 task IDs below are the *only* tasks that will be graded for Exhibit C. The list is fixed; no task may be added or removed after this run-plan is committed.
...
| version pin | git commit hash of `scouting/exhibit-c/run.py` at run time, written into `rerun_metadata.harness_commit` |
...
- Per-task cost is reconstructed from `tokens_in × input_rate + tokens_out × output_rate` using the rates in [`scouting/exhibit-c/price-table.yaml`](price-table.yaml) (price-table date 2026-05-03).
...
1. **NOW** — this run-plan, [`../exhibit-c-decision.md`](../exhibit-c-decision.md), and [`../../studies/exhibit-c.yaml`](../../studies/exhibit-c.yaml) are committed. No API calls have been made.
2. **Next** — vendor the 30-task subset to `scouting/exhibit-c/humaneval-tasks-30.jsonl` from upstream HumanEval.
3. **Next** — commit `scouting/exhibit-c/run.py`, `grade.py`, `normalize.py`.
