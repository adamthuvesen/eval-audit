...
- `eval_audit/__init__.py` `__version__` now reads from package metadata via
  `importlib.metadata.version("eval-audit")`, with `pyproject.toml` as the
  single source of truth and no hardcoded duplicate.
...
- Resolution planning: target_mde + bootstrap CI half-width to required-N
